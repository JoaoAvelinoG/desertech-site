...
import type { IpApiResponse } from "@/types/IpApiResponse";
...
export async function getLocationByIp(): Promise<IpApiResponse> {
  const response = await api.get<IpApiResponse>("/");
  return response.data;
}

interface Payload extends ContactFormData {
  city: string;
  region: string;
  region_code: string;
}
